...
namespace pga{
    ...
    template <typename T> struct simulation_ : ff::ff_node_t<RANGE>{
        ...
        RANGE* svc(RANGE* run) {
            ...
            for(int i = run->start; i < run->end; i++){
                P->current_population[i].simulate();
                if(P->norm_type == linear)run->cum_fitness += P->current_population[i].get_fitness();
                else if (P->norm_type == softmax) run->cum_fitness += exp(P->current_population[i].get_fitness());
            }
    ...
    template <typename T> struct reproduce_ : ff::ff_node_t<RANGE>{
        ...
        RANGE* svc(RANGE* run) {
            ...
            int index = run->start/(P->size/P->workers);
            if(index == 0)number_to_copy += extra_shift;

            for(int i = run->start; i < run->end; i++){
            // if we can't divide the agents to keep we add the spare part to the first worker
                if(i < number_to_copy){
                    P->new_population[i] = P->current_population[index * number_to_copy + i + extra_shift];
                }else{
                    int index1 = P->pick_random_parent();
                    int index2 = P->pick_random_parent();
                    P->new_population[i].reproduce(P->current_population[index1], P->current_population[index2]);
                }
            }
            this->ff_send_out(run);
            return this->GO_ON;   
    ...
    template <typename T> struct collector_emitter : ff::ff_node_t<RANGE>{
        ...
        RANGE* svc(RANGE* run) {
            ...
            // merging the results
            number_chunks++;
            P->cum_fitness += run->cum_fitness;
            delete run;
            #ifdef DEBUG
            std::cout<<"reciving from simuatioon\n";
            #endif // DEBUG

            if(number_chunks *(P->delta >= P->size)){
                ...
                for(int new_start = 0; new_start < P->size; new_start += step) {
                    this->ff_send_out(new RANGE(new_start, std::max(new_start + step, P->size)));
                }
                #ifdef DEBUG
    ...
    template <typename T> struct emitter : ff::ff_node_t<RANGE>{
        ...
        RANGE* svc(RANGE* run) {
            ...
            if(number_chunks < P->size/P->workers){
                return this->GO_ON;
            }     
            #ifdef GET_STATISTICS
            ff::ffTime(ff::STOP_TIME);
    ...
    void pga::ff_population_farm<T>::simulate(int iter){
        ...
        ff::ff_Farm<RANGE> simulation_farm(std::move(S));
        // the second farm
        ff::ff_Farm<RANGE> reprodcution_farm(std::move(R));
